download-node:
  archive.extracted:
    ...
    - name: /opt/node-v{{ salt['pillar.get']('node:version', '4.1.2') }}-linux-x64
    - archive_format: tar
    - tar_options: --strip 1
...
symlink-node:
  file.symlink:
    ...
    - target: /opt/node-v{{ salt['pillar.get']('node:version', '4.1.2') }}-linux-x64/bin/node

symlink-npm:
  file.symlink:
    - name: /usr/local/bin/npm
    - target: /opt/node-v{{ salt['pillar.get']('node:version', '4.1.2') }}-linux-x64/bin/npm
